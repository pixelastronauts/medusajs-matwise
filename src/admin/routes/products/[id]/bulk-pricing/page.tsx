...
const BulkPricingPage = () => {
  ...
  // Check if any variants have legacy metadata tiers
  const hasLegacyMetadata = product?.variants?.some(
    (v: any) => v.metadata?.volume_pricing_tiers?.length > 0
  )

  const clearAllMetadataTiers = async () => {
    if (!confirm("This will remove ALL legacy volume_pricing_tiers from ALL variants of this product. Are you sure?")) return
    
    setClearingMetadata(true)
    try {
      const response = await fetch(`/admin/volume-pricing/clear-all-metadata`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        toast.success("Success", {
          description: `Cleared metadata from ${data.variants_cleared} variant(s)`,
        })
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <Text className="text-sm font-semibold text-orange-800">
              ⚠️ Legacy Pricing Editor
            </Text>
            <Text className="text-sm text-orange-700 mt-1">
              This page edits <strong>metadata-based tiers</strong> which are used as a fallback. 
              For better control with customer groups and date ranges, use the new <strong>Volume Price Lists</strong> system.
            </Text>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {hasLegacyMetadata && (
              <Button 
                variant="danger"
                size="small"
                onClick={clearAllMetadataTiers}
                disabled={clearingMetadata}
              >
                {clearingMetadata ? "Clearing..." : "Clear All Metadata"}
              </Button>
            )}
            <Button 
              variant="primary"
